...
/**
 * Mapeamentos diretos conhecidos e canônicos de Produtores Rurais
 */
...
/**
 * Normaliza e padroniza a string de origem/produtor cruzando com mapeamentos
 * canônicos e com a coleção de Clientes/Produtores no Banco de Dados.
 * 
...
async function normalizeProducerOrigin(originStr, notesStr = '') {
  ...
  try {
    ...
    if (cleanSearch.length >= 3) {
      const regexStr = escapeRegex ? escapeRegex(cleanSearch) : cleanSearch.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      const regex = new RegExp(regexStr, 'i');
      const foundClient = await Client.findOne({ name: regex }).lean();
